...
int main(){
    // 调用函数并解析边界条件
    vector<BoundaryCondition> boundaryConditions = parseBoundaryConditions(filename);

    // 输出所有解析到的边界条件
    for (const auto& bc : boundaryConditions) {
        cout << "ZoneID: " << bc.zoneid 
             << ", BCType (hex): " << hex << bc.bctype 
             << " (" << getBoundaryType(bc.bctype) << ")"
             << ", Region: " << bc.regionName << endl;
    }
